...
- **Public Tier:** Houses the Application Load Balancer (ALB) and a Bastion host. The ALB handles incoming HTTP traffic from the internet.
...
- **AWS Systems Manager (SSM):** Used for secure, auditable access to private ec2 instances without needing an SSH gateway.
